...
> **84% 的学生已经在使用 AI 工具，但只有 18% 觉得自己能在职业场景中用好它们。**
> 这套课程就是为填平这条鸿沟而写。
>
> 473 节课，20 个阶段，约 320 小时。Python、TypeScript、Rust、Julia。
> 每一课都会交付一个可复用的产物：prompt、skill、agent、MCP server。
> 免费、开源、MIT 许可。
>
> 你不只是“学 AI”，你会把它从头到尾亲手“造出来”。

## 这套课程怎么学

大多数 AI 教材都是零散的：一篇论文、一篇微调笔记、一个花哨的 agent demo。它们很少真正对齐。
你能部署一个聊天机器人，却讲不清 loss 曲线代表什么；你能把一个工具函数接到 agent 上，却解释不了
“注意力机制”在模型内部到底做了什么。

这套课程就是“脊柱”：20 个阶段、473 节课、四种语言（Python、TypeScript、Rust、Julia）。
一端是线性代数，另一端是自治 swarm。每个算法都先从原始数学开始亲手实现：反向传播、分词器、
注意力机制、agent 循环。等你开始用 PyTorch 时，你已经知道它在底层到底做了什么。

每节课都遵循同一个循环：读问题、推导数学、写代码、跑测试、保留产物。
不靠五分钟短视频、不靠复制粘贴式部署、不手把手喂饭。免费、开源，并且能在你自己的笔记本上跑通。
...
20 个阶段层层叠加。数学是地基，agent 与工程化落地是屋顶。
如果你已经掌握底层内容，可以跳过；但不要跳过之后，又疑惑为什么上层东西一直坏。
...
每节课都遵循六段式节奏。*Build It / Use It* 的分界是脊柱：
你先把算法从零实现一遍，再用生产级库跑一遍同样的事。因为你自己写过“小版本”，所以你能理解框架在做什么。

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#fafaf5','primaryTextColor':'#1a1a1a','primaryBorderColor':'#3553ff','lineColor':'#3553ff','fontFamily':'JetBrains Mono','fontSize':'13px'}}}%%
flowchart LR
  M["MOTTO<br/><sub>一句话核心</sub>"] --> Pr["PROBLEM<br/><sub>具体痛点</sub>"]
  Pr --> C["CONCEPT<br/><sub>图示与直觉</sub>"]
  C --> B["BUILD IT<br/><sub>原始数学，不用框架</sub>"]
  B --> U["USE IT<br/><sub>用 PyTorch / sklearn 跑同一件事</sub>"]
...
[Contents](#contents) 里打开任意已完成课程。无需安装，也无需克隆仓库。
...
**方式 C：评估起点（推荐）。** 更聪明地跳到合适阶段。在 Claude、Cursor、Codex、OpenClaw、Hermes
或任何已安装本课程 skill 的 agent 里：

```bash
/find-your-level
```

10 个问题，把你的知识映射到起始阶段，并生成带时间估计的个性化学习路径。每完成一个阶段后：
...
- 你会写代码（任何语言都行，Python 会更顺）。
- 你想理解 AI **到底怎么工作的**，而不是只会调 API。

### 内置 agent skills（Claude / Cursor / Codex / OpenClaw / Hermes）

| Skill | 用途 |
|---|---|
| [`/find-your-level`](.claude/skills/find-your-level/SKILL.md) | 10 题分级测验：定位起始阶段，并生成带小时估计的个性化路径。 |
| [`/check-understanding <phase>`](.claude/skills/check-understanding/SKILL.md) | 按阶段测验：8 题，带反馈与建议回看的具体课程。 |
...
很多课程以“恭喜你学会了 X”结束；这里的每一课都会以一个**可复用工具**结束：你可以直接安装或粘贴进日常工作流。
...
> 中文说明：本仓库的课程目录与标题以英文为准（便于保持链接与自动同步一致）。本文件主要翻译叙述与说明部分。

### FIG_002 · A worked sample

Phase 14，第 1 课：agent 循环。大约 120 行纯 Python，无任何依赖。
...
用 `npx skills add` 安装这些产物。它们可以直接接入 Claude、Cursor、Codex、OpenClaw、Hermes，
或任何会读取 SKILL.md / AGENTS.md 的 agent。是真工具，不是作业。

### 把整套课程 skill 安装进你的 agent

本仓库在 `phases/**/outputs/` 下提供 382 个 skill 和 99 个 prompt。

**推荐：通过 [skills.sh](https://skills.sh) 安装。** 不用克隆仓库、不用 Python，
会自动识别你的 agent skills 目录：
...
`skills` 会写入你的 agent 会读取的目录：例如 `.claude/skills/`、`.cursor/skills/`、`.codex/skills/`、
OpenClaw 的 skills 目录、Hermes 的 bundle 路径，或任何支持 SKILL.md 的工具。一个命令，覆盖所有 agent。

**进阶：使用 `scripts/install_skills.py` 做离线安装或自定义布局。** 需要先克隆仓库。
当你需要按 tag 过滤、dry-run 预览，或使用非默认布局时很有用：
...
`<target>` 是你的 agent skills 目录（例如：`~/.claude/skills/`、`~/.cursor/skills/`、
`~/.config/openclaw/skills/`、`.skills/`，或任何你的 agent 会读取的路径）。

默认情况下脚本拒绝覆盖已存在的目标目录；它会列出所有冲突路径并以退出码 1 结束。
用 `--dry-run` 预览冲突，用 `--force` 强制覆盖。每一次非 dry-run 的执行都会在目标目录写入
`manifest.json`，其中按类型与 phase 分组列出完整清单。选择你 agent 能读取的布局：
...
### 把 Agent Workbench 注入你自己的仓库

Phase 14 的 capstone 提供了可复用的 Agent Workbench 套件（AGENTS.md、schemas、init / verify / handoff 脚本）。
你可以用下面的命令把它脚手架化到任意仓库：
...
你会得到已连线的 7 个 workbench 组件、一份起始 `task_board.json`，以及一份新的
`agent_state.json`（`schema_version: 1`）。接下来：编辑任务、编辑 `AGENTS.md`、运行
`scripts/init_agent.py`，把契约交给你的 agent。套件源码位于：
`phases/14-agent-engineering/42-agent-workbench-capstone/outputs/agent-workbench-pack/`.

### 用 JSON 浏览整套课程

`scripts/build_catalog.py` 会遍历磁盘上的每个 phase、每节课、每个产物，并在仓库根目录生成 `catalog.json`。
一个文件，包含课程的全部事实来源。
...
catalog 来自文件系统而不是 README，因此计数永远与磁盘实际内容一致。它可用于站点构建、下游工具，
或用于验证 README 的计数没有漂移。schema 在脚本顶部有说明。

GitHub Action（`.github/workflows/curriculum.yml`）会在每个 PR 上重建 `catalog.json`，
如果提交的文件过期就会让构建失败。你修改任何课程后都应该运行
`python3 scripts/build_catalog.py` 并提交结果，否则 CI 会拒绝 PR。同一工作流还会以 warn-only 模式运行
`audit_lessons.py`（不会因为既有漂移阻塞贡献者）。

### 对每节课的 Python 代码做 smoke-check

`scripts/lesson_run.py` 会对每节课的 `code/` 目录下所有 `.py` 文件进行字节码编译检查。
默认只做语法检查：不执行代码、不需要 API key，也不需要重型 ML 依赖。它能捕获贡献者最常引入的回归：
缩进错误、f-string 损坏、误编辑等。
...
`--execute` 会实际运行每节课的 `code/main.py`（或第一个 `.py` 文件），每节课超时时间 10 秒。
如果入口文件以 `# requires: pkg1, pkg2` 注释列出非 stdlib 依赖，该课会以 `needs <deps>` 的原因被跳过。
该脚本是可选工具，并没有接入 CI。

该脚本本身只依赖 stdlib，要求 Python 3.10+。可以设置 `LINK_CHECK_SKIP=domain1,domain2`
覆盖默认跳过列表（`twitter.com`、`x.com`、`linkedin.com`、`instagram.com`、`medium.com`：
这些域名通常会强力拦截自动化 HEAD/GET）。

## 从哪里开始

| 背景 | 从这里开始 | 预计时间 |
|---|---|---|
| 编程与 AI 都是新手 | Phase 0 — Setup | ~306 小时 |
| 会 Python，但 ML 新手 | Phase 1 — Math Foundations | ~270 小时 |
| 会 ML，但深度学习新手 | Phase 3 — Deep Learning Core | ~200 小时 |
| 会深度学习，想学 LLM 与 agents | Phase 10 — LLMs from Scratch | ~100 小时 |
...
只要有任何规则失败，退出码就会非 0。规则（L001–L010）会校验目录结构、
`docs/en.md` 是否存在且包含 H1、`code/` 是否非空、`quiz.json` schema 是否正确
（会拒绝曾导致 issue #102 的旧 `q/choices/answer` 键），以及课文档中的相对链接是否有效。
...
免费、MIT 许可、473 节课。课程完全依靠赞助维持，仅接受现金赞助。
...
| 档位 | $/月 | 你会得到 |
|------|------|---|
| Backer | $25 | 在 BACKERS.md 里署名 |
| Bronze | $250 | README 赞助区文字行 + 上线日推文 |
| Silver | $750 | README 小 logo + 在 API 课程中列为受支持 provider 之一 |
| Gold | $2,000 | README 中等 logo + 赞助页 + 每季度一次 X / LinkedIn 联合曝光 |
| Platinum | $5,000 | 首屏 Hero logo + 一节专门的集成课程（最多 1 个合作方） |
...
如果这份手册帮到了你，给仓库点个 star。它能让项目继续活下去。

## 许可协议

MIT。你可以随意使用：fork、教学、商业化、上线交付都可以。署名感谢但不强制。
